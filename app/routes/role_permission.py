...
from app.schema.role_permission import RolePermissionSchema
...
@router.post("/")
async def create_role_permission(data: RolePermissionSchema, credentials: str = Depends(TokenManager.verify_token)):
...
@router.put("/{id}")
async def update_role_permission(id: int, data: RolePermissionSchema, credentials: str = Depends(TokenManager.verify_token)):
    data = await objects.update(id=id, data=data.dict())
    return JSONResponse(content={"message":"success","data":data}, status_code=status.HTTP_200_OK)

@router.delete("/{id}")
async def delete_role_permission(id: int, data: RolePermissionSchema, credentials: str = Depends(TokenManager.verify_token)):
